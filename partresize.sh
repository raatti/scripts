...
# Tested with CentOS7, RHEL7. This script is only intended for MBR partitioned disks and not for GPT.
#
...
#
# 
# based on script found originally at: https://github.com/alpacacode/Homebrewn-Scripts
#
# modified somewhat to work on my use cases and automatically detect correct partition etc.
########
...
then
  ## p="/dev/sda2"
  ## first match LVM partition
  p=$(fdisk -l | grep -m 1 LVM | cut -f 1 -d ' ')
  ## CentOS default LVM partition
  l="/dev/centos/root"
  #usage
fi
...
diskname=$(echo "$p" | grep -oP "\/dev\/[a-z]{1,}")
echo "Disk = $diskname"
disksize=$(parted $diskname --script unit MB print | grep $diskname | cut -f 2 -d ':')
partsize=$(parted $diskname --script unit MB print | egrep "^\ 2" | awk '{print $4;}')
...
then
  ...
  if [ $oldsize -lt $newsize ] || [ "$disksize" != "$partsize" ];
  then
